<header x-data="{ open: false }"
    class="fixed top-0 left-0 right-0 z-50 h-20 bg-gradient-to-r from-cyan-600 to-teal-600 text-white shadow-lg">

    {{-- PERUBAHAN DI SINI: Ganti 'container' dengan 'max-w-7xl mx-auto px-4 sm:px-6 lg:px-8' --}}
    <div class="max-w-7xl mx-auto h-full flex items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <a href="{{ route('home') }}" class="flex items-center gap-2 md:gap-3 group">
            {{-- Logo 1 --}}
            <img src="{{ asset('sbadmin/img/logo raditya R white.webp') }}" alt="Raditya"
                class="h-8 md:h-10 object-contain">

            {{-- Divider --}}
            <span class="h-5 md:h-6 w-px bg-white/50"></span>

            {{-- Logo 2 --}}
            <img src="{{ asset('sbadmin/img/Logo Dia_White_2.webp') }}" alt="Diamart"
                class="h-6 md:h-8 object-contain opacity-90 group-hover:opacity-100 transition">
        </a>


        {{-- HAMBURGER (MOBILE) --}}
        <button @click="open = true" class="md:hidden text-3xl focus:outline-none">
            ☰
        </button>
        ...
        <nav class="hidden md:flex items-center gap-8 text-sm font-medium">
            ...
            @auth
                <div x-data="{ dropdown: false }" class="relative">
                    <button @click="dropdown = !dropdown" class="flex items-center gap-2 hover:text-gray-200 transition">
                    ...
                    <div x-show="dropdown" @click.away="dropdown = false"
                        ...
                        <a href="{{ route('profile.index') }}" class="block px-4 py-3 hover:bg-gray-100 transition">
                            Profile
                        </a>

                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button class="w-full text-left px-4 py-3 hover:bg-red-500 hover:text-white transition">
                                Logout
                            </button>
                        </form>
                    </div>
            ...
            @else
                <a href="{{ route('login') }}" class="px-4 py-2 rounded-lg bg-white/20 hover:bg-white/30 transition">
                    Login
                </a>
            @endauth
        </nav>
    ...
    {{-- OVERLAY --}}
    <div x-show="open" x-transition.opacity @click="open = false" class="fixed inset-0 bg-black/60 md:hidden">
    </div>
