    ...
    <div class="h-full flex justify-between items-center px-8">
        ...
        <nav class="hidden md:flex items-center space-x-6">
            ...
            <a href="{{ route('cart.index') }}" class="relative text-lg hover:text-gray-200">
                ...
                <span class="absolute -top-2 -right-2 bg-red-500 text-xs w-5 h-5 flex items-center justify-center rounded-full">
                    {{ session('cart_count') }}
                </span>
                @endif
            </a>
    ...
    <aside
        ...
        <div class="p-6 border-t border-gray-700">
            ...
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button
                    class="w-full bg-teal-600 hover:bg-teal-700 py-2 rounded-full text-sm">
                    Logout
                </button>
            </form>
            @else
